...
async function restoreBackup(backupPath) {
  ...
  if (!mongoUrl) {
    ...
    console.log(
      "   Please set MONGO_URL to your target database connection string",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    for (const collectionName of metadata.collections) {
      ...
      if (!fs.existsSync(backupFile)) {
        console.log(
          `   ⚠️  Skipping ${collectionName} - backup file not found`,
        );
        continue;
      }
...
if (!backupPath) {
  ...
  console.log(
    "Example: node scripts/restore-db.js backups/backup_2025-08-21T17-36-31",
  );
  process.exit(1);
}
