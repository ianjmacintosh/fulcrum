...
export class UserService {
  ...
  async createUser(userData: { email: string; name: string; hashedPassword: string }): Promise<User> {
    ...
    if (!validationResult.success) {
      throw new Error(`User validation error: ${validationResult.error.message}`)
    }
  ...
  async updateUser(id: string, updates: Partial<Pick<User, 'email' | 'name' | 'hashedPassword'>>): Promise<User | null> {
    const collection = await this.getCollection()
