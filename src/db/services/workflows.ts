...
import { Workflow, WorkflowSchema, ApplicationStatus, ApplicationStatusSchema } from '../schemas'
...
export class WorkflowService {
  ...
  private async getStatusesCollection(): Promise<Collection<ApplicationStatus>> {
    if (!this.statusesCollection) {
      this.db = await connectToDatabase()
      this.statusesCollection = this.db.collection<ApplicationStatus>('application_statuses')
    }
  ...
  async getWorkflows(userId: string, limit: number = 100, skip: number = 0): Promise<Workflow[]> {
    const collection = await this.getWorkflowsCollection()
    return await collection.find({ userId }).limit(limit).skip(skip).sort({ createdAt: -1 }).toArray()
  }

  async getWorkflowById(userId: string, id: string | ObjectId): Promise<Workflow | null> {
    const collection = await this.getWorkflowsCollection()
  ...
  async updateWorkflow(userId: string, id: string | ObjectId, updates: Partial<Workflow>): Promise<Workflow | null> {
    const collection = await this.getWorkflowsCollection()
    const objectId = typeof id === 'string' ? new ObjectId(id) : id
  ...
  async deleteWorkflow(userId: string, id: string | ObjectId): Promise<boolean> {
    const collection = await this.getWorkflowsCollection()
    const objectId = typeof id === 'string' ? new ObjectId(id) : id
  ...
  async getStatuses(userId: string, limit: number = 100, skip: number = 0): Promise<ApplicationStatus[]> {
    const collection = await this.getStatusesCollection()
    return await collection.find({ userId }).limit(limit).skip(skip).sort({ createdAt: -1 }).toArray()
  }
