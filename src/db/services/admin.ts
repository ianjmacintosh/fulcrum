...
export class AdminService {
  ...
  async createAdminUser(username: string, hashedPassword: string): Promise<AdminUser> {
    ...
    if (!validationResult.success) {
      throw new Error(`Admin validation error: ${validationResult.error.message}`)
    }
  ...
  async ensureDefaultAdmin(defaultUsername: string, defaultHashedPassword: string): Promise<void> {
