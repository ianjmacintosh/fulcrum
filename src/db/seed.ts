...
async function seedAdmin() {
  ...
  if (process.env.ADMIN_PASSWORD) {
    console.log(
      "🔐 Admin password set from environment variable: ADMIN_PASSWORD",
    );
    console.log("   Username:", defaultUsername);
  } else {
    console.log(
      "🔐 IMPORTANT: Save this temporary password (will not be shown again):",
    );
    console.log("   Username:", defaultUsername);
...
async function migrateTestUserDataToAlice(aliceUserId: string) {
  ...
  const statusResult = await db
    ...
    .updateMany({ userId: testUserId }, { $set: { userId: aliceUserId } });
  ...
  const totalMigrated =
    ...
    workflowResult.modifiedCount +
    jobBoardResult.modifiedCount +
    appResult.modifiedCount;

  if (totalMigrated > 0) {
    console.log(
      `✅ Migrated ${totalMigrated} records from test user to Alice:`,
    );
...
export async function seedDatabase(forceReseed: boolean = false) {
  ...
  try {
    ...
    const existingJobBoards = await db
      .collection("job_boards")
      .countDocuments();
    const existingApps = await db.collection("applications").countDocuments();

    const totalExistingRecords =
      existingStatuses + existingWorkflows + existingJobBoards + existingApps;

    if (totalExistingRecords > 0 && !forceReseed) {
      console.log(
        `✅ Database already contains ${totalExistingRecords} records:`,
      );
    ...
    if (forceReseed && totalExistingRecords > 0) {
      console.log(
        `🔄 Force re-seeding with ${totalExistingRecords} existing records`,
      );
    }
