...
export const addStatusDates: Migration = {
  ...
  description: 'Derive status dates from existing events and add date-based status tracking fields',
      ...
      for (const app of applications) {
        ...
        if (app.events && Array.isArray(app.events)) {
          for (const event of app.events) {
            const eventTitle = (event.title || event.eventType || '').toLowerCase()
            const eventDate = event.date
            
            if (!eventDate) continue

            // Map event titles to status dates
            if (eventTitle.includes('applied') || eventTitle.includes('application')) {
              if (!statusDates.appliedDate || eventDate < statusDates.appliedDate) {
                statusDates.appliedDate = eventDate
              }
            }
            
            if (eventTitle.includes('phone screen') || eventTitle.includes('phone call') || eventTitle.includes('screening')) {
              if (!statusDates.phoneScreenDate || eventDate > statusDates.phoneScreenDate) {
                statusDates.phoneScreenDate = eventDate
              }
            }
            
            if (eventTitle.includes('interview') || eventTitle.includes('round 1') || eventTitle.includes('first round') || eventTitle.includes('technical')) {
              if (!statusDates.round1Date || eventDate > statusDates.round1Date) {
                statusDates.round1Date = eventDate
              }
            }
            
            if (eventTitle.includes('round 2') || eventTitle.includes('second round') || eventTitle.includes('final round') || eventTitle.includes('onsite')) {
              if (!statusDates.round2Date || eventDate > statusDates.round2Date) {
                statusDates.round2Date = eventDate
              }
            }
            
            if (eventTitle.includes('accepted') || eventTitle.includes('offer') || eventTitle.includes('hired')) {
              if (!statusDates.acceptedDate || eventDate > statusDates.acceptedDate) {
                statusDates.acceptedDate = eventDate
              }
            }
            
            if (eventTitle.includes('rejected') || eventTitle.includes('declined') || eventTitle.includes('rejection') || 
                eventTitle.includes('not selected') || eventTitle.includes('passed')) {
              if (!statusDates.declinedDate || eventDate > statusDates.declinedDate) {
        ...
        if (!statusDates.appliedDate && app.events && app.events.length > 0) {
          const earliestEvent = app.events.reduce((earliest: any, event: any) => {
        ...
        const hasNewDates = Object.keys(statusDates).some(key => statusDates[key] && !app[key])
        ...
        if (hasNewDates) {
          ...
          if (Object.keys(updateDoc).length > 0) {
            ...
            if (dryRun) {
              console.log(`   [DRY RUN] Would add status dates for application ${app._id}:`, Object.keys(updateDoc))
            }
          }
        }
      }
