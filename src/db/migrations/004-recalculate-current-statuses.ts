...
export const recalculateCurrentStatuses: Migration = {
  ...
  description:
    "Recalculate currentStatus field based on status dates for all applications",

  async execute(
    db: import("mongodb").Db,
    dryRun: boolean,
  ): Promise<import("./migration-runner").MigrationResult> {
    ...
    try {
      if (!dryRun) {
        ...
        console.log(
          `✅ Successfully updated currentStatus for ${updatedCount} applications`,
        );

        if (updatedCount === 0) {
          console.log(
            "ℹ️  No applications needed status updates (all were already correct)",
          );
        }
        ...
        console.log(
          "[DRY RUN] Would recalculate current statuses for all applications",
        );
        return {
          success: true,
  ...
  async rollback() {
    return {
      success: false,
      message:
        "Cannot rollback current status recalculation - this migration only fixes incorrect data",
      documentsModified: 0,
