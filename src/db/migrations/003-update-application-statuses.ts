...
export const updateApplicationStatuses: Migration = {
  ...
  description: 'Migrate to new 7-status workflow and create default statuses for all users',
  ...
  async execute(db: Db, dryRun: boolean): Promise<MigrationResult> {
    ...
    try {
      ...
      for (const userId of userIds) {
        ...
        const existingStatuses = await statusesCollection.find({ userId }).toArray()
        ...
        if (!hasNewWorkflow) {
          ...
          const statusesToCreate = defaultStatuses.filter(ds => 
            !existingStatusNames.includes(ds.name)
          )
          ...
          if (statusesToCreate.length > 0) {
            ...
            if (dryRun) {
              console.log(`   [DRY RUN] Would create ${statusesToCreate.length} statuses for user ${userId}`)
            }
          }
        }
      }
      ...
      for (const app of applications) {
        ...
        if (statusDoc && app.currentStatus.id !== statusDoc._id.toString()) {
          ...
          if (dryRun) {
            console.log(`   [DRY RUN] Would update currentStatus.id for application ${app._id}`)
          }
        }
      }
  ...
  async rollback(db: Db): Promise<MigrationResult> {
    try {
      ...
      return {
        ...
        errors: ['WARNING: This rollback removed workflow statuses. Applications may have invalid status references.']
      }
