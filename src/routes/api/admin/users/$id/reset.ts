...
export const ServerRoute = createServerFileRoute('/api/admin/users/$id/reset').methods({
  POST: async ({ request, params }) => {
    ...
    try {
      ...
      const preserveCustomJobBoards = formData.get('preserveCustomJobBoards') === 'true'
      ...
      const beforeSummary = await userOnboardingService.getUserDataSummary(userId)
      
      // Perform the reset
      await userOnboardingService.resetUserData(userId, resetOptions)
      
      // Get summary after reset
      const afterSummary = await userOnboardingService.getUserDataSummary(userId)
      ...
      if (!resetOptions.preserveCustomJobBoards && beforeSummary.hasCustomJobBoards) {
