...
export const ServerRoute = createServerFileRoute("/api/application-statuses/")
  ...
  .methods({
    GET: async ({ context }) => {
      ...
      try {
        console.log(
          "Application Statuses API: Fetching statuses for userId:",
          auth.user.id,
        );
        const statuses = await applicationStatusService.getAllStatuses(
          auth.user.id,
        );
        ...
        if (statuses.length === 0) {
          console.log(
            "No statuses found, creating default statuses for user:",
            auth.user.id,
          );
          const defaultStatuses =
            await applicationStatusService.createDefaultStatuses(auth.user.id);
        ...
        console.error(
          "Application Statuses API: Error fetching statuses:",
          error,
        );
