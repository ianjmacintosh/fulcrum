...
describe('requireUserAuth', () => {
  ...
  test('throws redirect for unauthenticated user on client-side', async () => {
    ...
    ).rejects.toThrow('REDIRECT: {"to":"/login","search":{"redirect":"http://localhost:3000/applications"}}');
  });
...
describe('requireAdminAuth', () => {
  ...
  test('preserves redirect URL in search params', async () => {
    ...
    ).rejects.toThrow('REDIRECT: {"to":"/login","search":{"redirect":"http://localhost:3000/admin/users?tab=active"}}');
  });
});
